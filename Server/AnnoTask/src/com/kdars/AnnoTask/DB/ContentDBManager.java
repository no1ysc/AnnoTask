...
public class ContentDBManager {
	...
	public ArrayList<Integer> getJobCandidates() {
		...
		jobCandidates = contentDB.checkUpdates("working_status", 0); // colName,
																		// value,
																		// status
	...
	public ArrayList<Document> getDocIDsFromDate(String startDate,
		...
		return contentDB
				.queryFromDate(startDate, endDate, bNaver, bDaum, bNate);
	}

	// (기흥) phase2.5 - 구현 계획1
	public ArrayList<Integer> getClientJobCandidates() {
		return contentDB.queryClientJobCandidates(ContextConfig.getInstance()
				.getClientJobUnit());
	}
	...
	// (기흥) 서버 문서 수 제한을 위한 코드
	public boolean isJobCandidatesEnough() {
		int nReadyDoc = contentDB.queryCompleteJobCount();
		if (nReadyDoc >= ContextConfig.getInstance()
				.getServerHoldDocumentCount()) {
			return true;
		}
		return false;
	}
